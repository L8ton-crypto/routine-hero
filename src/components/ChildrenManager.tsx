...
export default function ChildrenManager({ familyChildren, familyId, onRefresh }: Props) {
  const [editingChild, setEditingChild] = useState<Child | null>(null)
  const [isAddingChild, setIsAddingChild] = useState(false)

  const addChild = async (childData: { name: string; age: number; avatar: string }) => {
    try {
      await fetch('/api/children', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ familyId, ...childData })
      })
  ...
  const updateChild = async (child: Child) => {
    try {
      await fetch('/api/children', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: child.id, name: child.name, age: child.age, avatar: child.avatar })
      })
  ...
  const resetProgress = async (childId: number) => {
      ...
      try {
        ...
        if (child) {
          await fetch('/api/children', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: childId, name: child.name, age: child.age, avatar: child.avatar })
          })
          // Note: XP/level/streak reset would need a dedicated endpoint
          // For now this just refreshes
          onRefresh()
        }
      } catch (e) {
            ...
            <div key={child.id} className="bg-white/10 rounded-2xl p-6 space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="w-full bg-white/20 rounded-full h-3 overflow-hidden">
                  <div className="bg-gradient-to-r from-yellow-400 to-orange-500 h-full transition-all duration-500"
              ...
              <button onClick={() => resetProgress(child.id)}
                className="w-full bg-yellow-500/20 hover:bg-yellow-500/30 text-white py-2 rounded-lg text-sm font-semibold transition-all">
                Reset Progress
              </button>
            </div>
          ))}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 max-w-md w-full border-2 border-white/20">
            ...
            <ChildForm
              ...
              onCancel={() => { setEditingChild(null); setIsAddingChild(false) }}
            />
          </div>
        </div>
      )}
...
function ChildForm({ initialChild, onSave, onCancel }: {
      ...
      <div>
        ...
        <div className="grid grid-cols-8 gap-2">
          {AVATAR_OPTIONS.map((avatar) => (
            <button key={avatar} type="button" onClick={() => setFormData({ ...formData, avatar })}
